...
#         # initialize buffers
#         self.base_lin_vel = torch.zeros((self.num_envs, 3), device=gs.device, dtype=gs.tc_float)
#         self.last_base_lin_vel = torch.zeros((self.num_envs, 3), device=gs.device, dtype=gs.tc_float)  # 新增：上一帧线速度，用于计算加速度
#         self.base_ang_vel = torch.zeros((self.num_envs, 3), device=gs.device, dtype=gs.tc_float)
#         self.projected_gravity = torch.zeros((self.num_envs, 3), device=gs.device, dtype=gs.tc_float)
#         self.global_gravity = torch.tensor([0.0, 0.0, -1.0], device=gs.device, dtype=gs.tc_float).repeat(
#             self.num_envs, 1
...
#         self.robot.set_quat(self.base_quat[envs_idx], zero_velocity=False, envs_idx=envs_idx)
#         self.base_lin_vel[envs_idx] = 0
#         self.last_base_lin_vel[envs_idx] = 0  # 重置上一帧线速度
#         self.base_ang_vel[envs_idx] = 0
#         self.robot.zero_all_dofs_velocity(envs_idx)
...
#         # 惩罚机器人姿态（Roll-Pitch-Yaw）的不稳定性
#         # 获取弧度制的俯仰(pitch)和横滚(roll)角度 - 转换为弧度
#         pitch_rad = torch.abs(self.base_euler[:, 1] * math.pi / 180.0)
#         roll_rad = torch.abs(self.base_euler[:, 0] * math.pi / 180.0)
        
#         # 计算俯仰和横滚角度的平方和作为惩罚
#         # 加权可以根据实际情况调整，这里给予相同权重
#         return torch.square(pitch_rad) + torch.square(roll_rad)
    
#     def _reward_lin_acc_z(self):
#         # 惩罚垂直加速度，抑制跳跃行为
#         # 计算Z方向加速度（使用速度差值估计）
#         z_acc = (self.base_lin_vel[:, 2] - self.last_base_lin_vel[:, 2]) / self.dt
        
#         # 返回Z方向加速度的平方作为惩罚
#         return torch.square(z_acc)